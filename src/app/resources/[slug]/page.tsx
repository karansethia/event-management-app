    ...
    <main className='w-full relative h-screen overflow-y-scroll scrollbar-hidden scroll-smooth'>
      ...
      <section className='w-full relative snap-center snap-mandatory space-y-10 py-10 max-w-6xl mx-auto'>
        <div className='flex flex-col gap-3 w-full'>
          ...
          <h1 className='font-header text-xl sm:text-3xl xl:text-5xl font-semibold tracking-wide pt-5'>{resourceData.blog_title}</h1>
          <div className='w-full flex items-center gap-2 pb-16'>
            {resourceData.categories.map(cat => (
              <Badge key={cat.id} className='rounded-full font-content font-semibold tracking-wide'>{cat.category_name}</Badge>
            ))}
          </div>
